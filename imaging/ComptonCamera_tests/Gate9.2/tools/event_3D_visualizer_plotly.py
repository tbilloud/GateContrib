...
class MainWindow(QMainWindow):
    ...
    def create_plotly_figure(self, event_data):
        ...
        fig.update_layout(
            legend_title_text="",
            scene=dict(
                xaxis=dict(title="localPosX", range=new_x_range),
                yaxis=dict(title="localPosY", range=new_y_range),
                zaxis=dict(title="localPosZ", range=new_z_range)
            )
        )
        return fig
